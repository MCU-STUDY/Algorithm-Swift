...
struct Heap<T: Comparable> {
    ...
    public mutating func pop() {
        ...
        var currentIndex = 1
          ...
          while true {
              // 가장 큰 자식 노드
              var samllerChildIndex = currentIndex
              ...
              if leftChildIndex < heap.count && heap[leftChildIndex] < heap[samllerChildIndex] {
                  samllerChildIndex = leftChildIndex
              }
              
              // 배열의 범위를 벗어나지 않고, 오른쪽 노드가 더 큰 경우
              if rightChildIndex < heap.count && heap[rightChildIndex] < heap[samllerChildIndex] {
                  samllerChildIndex = rightChildIndex
              }
              
              // 자식들보다 크거나 같은 상태
              if samllerChildIndex == currentIndex {
                  break
              }
              
              heap.swapAt(currentIndex, samllerChildIndex)
              currentIndex = samllerChildIndex
          }
    }
}
...
for _ in 0..<input {
    ...
    if number == 0 {
        ...
        heap.insert(element: number)
    }
}

// 메모리 : 81080 KB, 시간 : 124 ms
